...
**Etsy and Teachers Pay Teachers — deprioritized (decision made July 18, 2026).** Both require a setup/listing fee paid upfront, before any sales — not worth it given the product hasn't proven post-Gumroad demand on a new platform yet. **KDP paperback remains the only active expansion channel** (zero upfront cost, print-on-demand). Etsy/TpT can be revisited later if paperback performs well and there's appetite to pay in for additional discovery traffic.
...
**Reality check on the Mirror Room (noted July 18, 2026):** the community currently has only one other participant besides Mohan himself — soft-launching the companion app there will validate the mechanism works for at least one outside real user, but won't produce meaningful "organic multi-week return usage" data at any scale. Worth keeping expectations calibrated: this is a first real-world smoke test, not the volume needed to evaluate the v2/v3 gating conditions in Section 5. Growing the Mirror Room itself (or finding another small initial audience) may need to happen before the gating conditions can realistically be met.
...
3. Etsy and Teachers Pay Teachers are **not** being pursued — both require upfront setup fees before any sales, deprioritized in favor of KDP's zero-upfront-cost model (Section 2a)
